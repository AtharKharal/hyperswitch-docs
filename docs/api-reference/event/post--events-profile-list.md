...
**Endpoint:** `POST /events/profile/list`

List all Events associated with a Profile.

**Operation ID:** `List all Events associated with a Profile`

## Playground

<a href="https://sandbox.hyperswitch.io/playground?endpoint=/events/profile/list&method=POST" class="md-button-playground" target="_blank">Try in Playground →</a>
...
```bash
curl -X POST "https://sandbox.hyperswitch.io/events/profile/list" \
...
url = "https://sandbox.hyperswitch.io/events/profile/list"
...
const url = "https://sandbox.hyperswitch.io/events/profile/list";
const options = {
  method: "POST",
  url: url,
  headers: {"Content-Type": "application/json", "api-key": "YOUR_API_KEY"},
